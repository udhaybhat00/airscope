...
- **Reference docs** (open as needed): `docs/SUPPORTED-HARDWARE.md` (+ the grading process: `docs/GRADING.md`).

## Porting Session Cheatsheet

- **Porting code style**: When porting or working within `src/airscope/chips/**/*`: `docs/porting/CODE-STYLE.md`.
...
# Run
uv run airscope                     # or: uv run python -m airscope
...
uv run textual run --dev src/airscope/ui/app.py
```
...
Airscope is a userland 802.11 auditing tool. It communicates directly with USB wireless devices via **PyUSB**.
...
```
ui/      Textual screens (Splash → Scanner → Focus); AirscopeApp holds the DeviceManager + active interface
...
1. Create `src/airscope/chips/<name>/` with at minimum `__init__.py`, `driver.py`, `transport.py`, `constants.py` (+ `firmware.py` if the chip needs a FW upload).
2. `chips/<name>/__init__.py` declares the hardware, and must NOT import `driver.py` at module top:
   - `SUPPORTED_IDS: list[DeviceID]` (`from airscope.models import DeviceID`): every VID:PID this driver claims, with a human-readable description and any chip-id discriminator in `extras={}`.
   - `def import_driver()`: the one heavy import, lazy (`from .driver import <Class>; return <Class>`).
3. `driver.py` must subclass the `Driver` ABC (`airscope.chips.driver`); Python enforces the surface at instantiation:
