...
from airscope.campaigns.wps import known_pins, pins
from airscope.campaigns.pin import WpsCampaign, _state_path
from airscope.campaigns.wps.pixie import PixieBundle
from airscope.campaigns.wps.registrar import AttemptOutcome, PinResult
...
def _iface():
    ...
    def _lease(channel=None, fake_mac=None, bssid=None, ack_tally=False, iface=None):
        from airscope.wlan.lease import Lease
...
def test_lock_backoff_grows_with_observation():
    from airscope.campaigns.wps.lock import LockTracker
...
def test_load_run_state_roundtrip_and_missing(tmp_path):
    from airscope.campaigns.pin import load_run_state
...
def test_run_progress_line_cracked_is_silent():
    from airscope.campaigns.pin import CampaignState, run_progress_line
...
def test_known_pins_for_generates_valid_candidates():
    from airscope.campaigns.wps.known_pins import known_pins_for
