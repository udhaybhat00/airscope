...
from airscope.device.manager import device as find_device
...
def _winusb_dir() -> Path:
    return Path(tempfile.gettempdir()) / "airscope_winusb"
...
class SetupWindows(Setup):
    ...
    async def install(self, device_id: DeviceID, ui: Prompter) -> DeviceID | None:
        ...
        from airscope.ui.wiffy import INSTALL_LINES
    ...
    async def uninstall(self, device_id: DeviceID, ui: Prompter) -> SetupResult:
        ...
        from airscope.ui.wiffy import UNINSTALL_LINES
        ui.status(f"Removing airscope driver for {device_id.description}…")
