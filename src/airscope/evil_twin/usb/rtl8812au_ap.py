...
Register addresses and H2C command formats extracted from:
  - https://github.com/aircrack-ng/rtl8812au (kernel driver)
  - rtl8812a/rtl8812a_hal_init.c, core/rtw_ap.c, include/rtw_reg.h
  - https://github.com/janlueders/rtl8812au-macos (userland port)
"""
...
# ---------------------------------------------------------------------------
_USB_REQ_READ    = 0x05  # Read register/memory
_USB_REQ_WRITE   = 0x05  # Write register/memory
_USB_REQ_READ_FW = 0x06  # Read firmware memory
...
# RCR_APM is the physical-match bit alias
...
# ---------------------------------------------------------------------------
# H2C (Host-to-Chip) command IDs (from include/rtw_h2c.h)
# ---------------------------------------------------------------------------
H2C_MEDIA_STATUS_RPT = 0x01
H2C_SET_MACID        = 0x04
H2C_SET_BSSID        = 0x05
H2C_SET_BCN_VALID    = 0x07
H2C_SET_BC_CTRL      = 0x08
H2C_OP_MODE          = 0x22  # Set operation mode (AP/STA/etc.)
...
# TX descriptor field offsets (little-endian 32-bit words)
_TXDESC_WORD0 = 0   # PKT_TX: usb_tx_agg_mode, pkt_type, offset, etc.
_TXDESC_WORD1 = 4   # DATA_RETRY, RTS/CTS rate
_TXDESC_WORD2 = 8   # TX packet offset, pkt_len
_TXDESC_WORD3 = 12  # PACKET_ID (mac_id), queue_sel
_TXDESC_WORD6 = 24  # RX packet length / drain sequence
_TXDESC_WORD7 = 28  # Rate control settings
...
class Rtl8812auAP:
    ...
    def _config_beacon_timing(self):
        ...
        # DTIM period = 1
        write_reg_byte(self.dev, REG_BCNTCFG, 0x00)  # Default beacon config
        write_reg_byte(self.dev, REG_BCNTCFG + 1, 0x10)
        # Beacon TX descriptor head/tail
        write_reg(self.dev, REG_BCN_CTRL, 0x10)  # Enable beacon TX

    def _config_mac_port(self):
        """Configure MAC port registers for AP mode."""
        # Enable MAC0 port
        write_reg(self.dev, REG_PSR, 0x00)  # Select page 0
        # Set MAC function enable
    ...
    def _add_addr_cam(self, entry: int, client_mac: bytes, bssid: bytes):
        ...
        CAM format (8 bytes):
          ...
          Byte 6: flags (valid bit, key type)
          Byte 7: BSSID index / port
        """
    ...
    def _h2c_sta_assoc(self, mac_id: int, client_mac: bytes):
        ...
        data[2:6] = client_mac[:4]  # MAC address (first 4 bytes in H2C)
    ...
    def download_beacon(self, beacon_frame: bytes):
        ...
        The firmware will transmit this beacon at hardware-timed intervals
        instead of relying on software-timed injection.
        """
        # Write beacon to the beacon TX buffer
        bcn_buf_addr = 0x0200  # Beacon buffer address in firmware memory
    ...
    def diagnose(self) -> bool:
        ...
        print("  [3/5] Configuring RX filter... ", end="")
        try:
            self._set_rx_filter(RCR_AP_MODE)
            actual = read_reg(self.dev, REG_RCR)
            if actual & RCR_APM:
        ...
        print("  [5/5] Setting MAC address... ", end="")
        try:
