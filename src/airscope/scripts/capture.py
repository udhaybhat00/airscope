...
class Capture:
    ...
    def __init__(self, target=None, client=None, run_air=True, fast_hop=False,
        ...
        self.temp_dir_obj = tempfile.TemporaryDirectory(prefix="airscope_cap_", dir=shm)
    ...
    def airodump_segment(self, duration=20.0):
        ...
        the same 250 ms cadence airscope uses. `--band abg` = all bands (a=5 GHz,
    ...
    def fast_hop_segment(self, dwell=0.25, duration=12.0, channels=(1, 6, 11)):
        """Hop at the airscope TUI's pathological 0.25 s cadence to test whether
    ...
    def cleanup(self):
        ...
        if self.chipset == "unknown":
            ...
            if tmp_pcap.exists():
                salvage = Path(f"/tmp/airscope_unsaved_{int(time.time())}")
    ...
    def run(self):
        self.logger.log_main("--- Airscope Automated Capture Tool ---")
...
def main():
    parser = argparse.ArgumentParser(
        description="Airscope automated USB capture tool (run as root).")
