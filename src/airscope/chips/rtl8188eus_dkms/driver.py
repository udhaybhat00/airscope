...
Registered in ``wlan/discovery.py`` behind ``AIRSCOPE_RTL8188`` — the mainline-derived
``rtl8188eus`` stays the default for 2357:010c until this vendor port is hardware-proven to
tie/beat it on 2.4 GHz breadth; ``AIRSCOPE_RTL8188=dkms`` opts in. Exercise via
...
class Rtl8188eusDkmsDriver(Driver):
    ...
    async def _bringup(self, progress_cb: Optional[ProgressCallback] = None) -> bool:
        ...
        dig_enabled = self.enable_dig and os.environ.get("AIRSCOPE_RTL8188_DIG") != "off"
        if dig_enabled:
