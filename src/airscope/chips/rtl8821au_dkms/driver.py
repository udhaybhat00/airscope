"""RTL8821AU / RTL8811AU driver — vendor (contributor DKMS) cleanroom port.
...
``chips/rtl8821au/``, ordered by ``$AIRSCOPE_RTL8821`` — **this DKMS port is the default**;
...
class Rtl8821auDkmsDriver(Driver):
    ...
    async def set_channel(self, channel: int, scan: bool = False) -> bool:
        ...
        and airscope always calls ``set_channel(scan=False)`` before injecting, so skipping
