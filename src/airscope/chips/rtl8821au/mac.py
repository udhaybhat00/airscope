...
def is_chip_warm(transport: RTL8821AUTransport) -> bool:
    ...
    previous airscope session would leave behind.
    """
    from .constants import (
        BIT_MACRXEN,
        BIT_MACTXEN,
...
def apply_monitor_rx_filter(transport: RTL8821AUTransport) -> None:
    ...
    - The kernel init leaves REG_RCR configured for an associated STA: not
      ...
      ops->configure_filter, which airscope never gets.
    - A WARM chip inherited from a previous session (kernel STA driver, or an
      old airscope build) likewise has a non-monitor RCR, and the warm path skips
    ...
    # Read back so a AIRSCOPE_LOG run shows the actual runtime RX-filter state vs
