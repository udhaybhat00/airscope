"""RTL8812AU / 2T2R driver — vendor (vendor DKMS) cleanroom port.
...
vendor's monitor opmode + set-channel RX-START tail. It then starts the bulk-IN RX
reader (promiscuous monitor frames + per-frame 8812a RSSI) and the runtime 2-path
DIG/AGC watchdog. No IQK — vendor receives without it.
...
``AIRSCOPE_RTL8812=mainline`` to fall back to the mainline driver. ``inject_frame`` (2.4 GHz
TX: deauth /
...
_DEFAULT_CHANNEL = 1     # connect-time tune target (matches vendor's cold-boot capture)
...
class Rtl8812auDkmsDriver(Driver):
    ...
    async def connect(self, progress_cb: Optional[ProgressCallback] = None) -> bool:
        ...
        # Deterministic init chain — byte-for-byte with verify_pcap.py. No IQK (vendor
        ...
        # vendor's monitor opmode + set-channel RX-START tail: the channel re-tune that
        ...
        # vendor's tail leaves RCR = 0x90000001 (AAP|APP_PHYST|APPFCS — airmon's exact
        # state, leaning on RXFLTMAP). That value does NOT deliver management/broadcast
        # frames (beacons) into airscope's RX pipeline, so re-open the filter to airscope's
    ...
    async def _inject_frame(self, frame_bytes: bytes) -> bool:
        ...
        No byte-for-byte gate backs this path — vendor's cold-boot captures contain no
