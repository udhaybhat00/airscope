...
def mt76x02_mac_shared_key_setup(
    ...
    cleared state used by airscope's cold-boot init loop). Real key
    installation (TKIP / CCMP) isn't ported — airscope doesn't push keys to
    the chip; we do crypto in software for the WEP suite.
    """
    if key is not None:
        raise NotImplementedError(
            "shared_key_setup: keyed install not ported; airscope only uses "
