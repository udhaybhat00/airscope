"""RTL8814AU driver — vendor (vendor DKMS) cleanroom port.
...
``rtw88_8814au`` port loads only when ``AIRSCOPE_RTL8814=mainline`` is set in the
...
class Rtl8814auDkmsDriver(Driver):
    ...
    def _inject(self, t, frame_bytes: bytes, *, hw_rate: int = DESC_RATE1M,
        ...
        ``hw_rate``/``rate_id`` default to the fixed CCK-1M management rate the tool's own deauths
