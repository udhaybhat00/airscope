"""Firmware load for the RT5372 (RT5392).

The blob in ``assets/rt5372_fw.bin`` is the **second** 4 KB of linux-firmware's
...
(RT5392 included) [SRC rt2800usb.c:218-244]; we pre-extract the RT5392 section so
...
_FW_PATH = Path(__file__).parent / "assets" / "rt5372_fw.bin"
...
def _write_firmware(t: RT5372Transport, blob: bytes) -> None:
    ...
    if t.autorun_detect():
        ...
        # RT5392 ⇒ offset 4096, length 4096; the asset is already that section.
...
def upload(t: RT5372Transport, blob: bytes) -> None:
    ...
    if not t.wait_csr_ready():
        raise IOError("rt5372: unstable hardware (MAC_CSR0 not ready before FW load)")
    ...
    for _ in range(C.REGISTER_BUSY_COUNT):
        ...
        raise IOError("rt5372: PBF system register not ready after FW load")
