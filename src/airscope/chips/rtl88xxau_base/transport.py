"""RTL88xxAU USB transport — Realtek rtw88-family vendor control transfers.
...
so the bring-up code runs unchanged against either real hardware or a pcap. Identical
across the 88xxA family (8821au/8812au); the bulk-OUT MGMT-queue endpoint default
(0x09) is overridable per chip for TX.
"""
...
_DEFAULT_BULK_OUT_EP = 0x09  # NORMAL/MGMT-queue bulk-OUT endpoint (88xxA USB)


class Rtl88xxauTransport:
    def __init__(self, dev: usb.core.Device, bulk_out_ep: int = _DEFAULT_BULK_OUT_EP):
        self.dev = dev
    ...
    # --- bulk OUT (firmware uses control writes; bulk-OUT is TX) ---
        ...
        for intf in cfg:
            for ep in intf:
                if (usb.util.endpoint_direction(ep.bEndpointAddress)
                    ...
                    return self._in_ep
        raise RuntimeError("RTL88xxAU: no bulk-IN endpoint on the active interface")
